<script lang="ts">
	...
	export let user: IUser | any = $session
		...
		<div class="card">
			<div class="card-body">
				...
				<form on:submit|preventDefault={update}>
					<div class="row">
						<div class="col-sm-6">
							{#each [{ name: 'Nombre', value: user.firstName }, { name: 'Apellido', value: user.lastName }] as { name, value }}
								<div class="mb-3">
									<label for={name}>{name}</label>
									<input id={name} class="form-control" bind:value />
								</div>
							{/each}
